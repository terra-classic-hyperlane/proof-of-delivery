...
No TC a tarifa vive no IGP compartilhado (default hook do mailbox) — sem ajuste,
os RECIBOS também pagariam $0,08 e devorariam a comissão (BSC→TC e SOL→TC ficavam
negativos; nas outras chains o markup está no IGP específico do warp e recibos já
usam o hook oficial barato). Correção: `SendReceipt{gas_limit}` no vault — o
metadata do IGP (32B BE) cobra só o gás real de entrega (~100 LUNC p/ BSC, ~20 p/
SOL). O warp não expõe metadata → usuário segue pagando $0,08. O claim-agent cota
o IGP dinamicamente (`quote_gas_payment`, +2%) — sem valores fixos. Migração:
`deploy/tc-migrate-vault-gas-recibo.sh` (mesmo endereço, pool preservado).

**P&L do operador por transferência** (preços de 20/08): TC→SOL +$0,079 ·
TC→BSC +$0,065 · SOL→TC +$0,075 · BSC→TC +$0,067.
