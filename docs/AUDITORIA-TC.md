# Audit Record — Terra Classic Deploy (Phases 1–2)

**Date:** 2026-08-18 · **Chain:** columbus-5 · **Deployer:** `terra1run9wz09uhh6pu7ggcwwetrgye4wu7wn26mawp`
**Source code:** this repository (github.com/terra-classic-hyperlane/proof-of-delivery)
**Build:** `cosmwasm/optimizer:0.17.0` over versioned locks (reproducible — see README §Build)

## Stored codes

| Contract | code_id | SHA-256 (= on-chain data_hash, verified at deploy) | Store TX |
|---|---|---|---|
...
| Contract | Address | Instantiate TX |
|---|---|---|
...
Instantiation parameters: governor `{owner: deployer, oracle: terra1j8xzgzk…cwqj0d,
...
denom: uluna, reward: 50000000 (50 LUNC), window: 200000 blocks}`.
...
| 2 | governor: `claim_oracle_ownership` (ownership effected) | `EDE721139C5C205924FF74BDA067D435A6EBEC8DD93F06F8929901F65A80BD97` |
...
| 7 | Pool seed: 5,000 LUNC (BankSend) → vault | `B55FD50BC7473F8D96B65B43A8879E78BB4676C66A6741D8A04463DF277844C7` |

Bounds derived from the CURRENT values in the production oracle at deploy time
(current ÷3 · ×3) — current values read: dom 1 = 376·1e10 · dom 56 = 1098·3e9 ·
...
# 1. on-chain data_hash == sha256 in this table == rebuild from source:
...
# 3. current state:
terrad q wasm contract-state smart terra1j8xzgzk7vds5uzrplmnln4vcz6f205t9atdyflypzrr43cd5eh7scwqj0d '{"ownable":{"get_owner":{}}}' --node $NODE           # oracle owner = governor
...
terrad q wasm contract-state smart terra1gqkrh2va5mqdrlp90ez6lc2hgagxqju6fc7md4kldlz8lap9w4usduzc2q '{"solvency":{}}' --node $NODE                        # pool/capacity
...
Post-deploy verification executed on 08/18/2026: oracle owner = governor ✓ ·
...
**Migration on the SAME address** (`terra1gqkrh2…duzc2q`), signed by admin
`terra1run9wz…`. Pool and beneficiary preserved. Reproducible build
...
| set_remote_operators (attester terra1run9wz…, quorum 1) | `322620CAB36F631B76AE2FB5A711CE8F7CADB8EEB86E45D563AD9CEE0BA6F821` |
| Solana binding → `PbEo7Fn2…cwwrkS` | `3526970A18F1A1C66565E42D7ABEB9ED95D991B1BB815ED462613790D40212BE` |
| BSC binding → `0x8f085bad…5291` | `51763033E7BE6166C881F45BB60D4A4FBBB6F522CAC214BBAECDE20EB6C07F05` |
| ETH binding → `0xef818120…00ae` | `04174C7EB2608C66AF0303B93B49573E499199369B5A485C4B38291498159908` |
| reward 33 LUNC dom SOL / BSC / ETH | `E7D9F83A…11DBC8` · `AFF7068C…6A2DA8` · `0CCF3BC3…3EAE9B` |
| **attest delivery SOL** `1e070a74…` (+33 LUNC) | `844240A4154A672580A58F78EE5F33BEA751F5895DD52C2333851C3C2080CA95` |
| **attest delivery BSC** `72f1099d…` (+33 LUNC) | `4F687166BB2A155F34A4558BFA034455BBD57D4B028EB7043B76CE784AC1921B` |
| **attest delivery ETH** `6c6518b0…` (+33 LUNC) | `44E0B62DA58CF8ADCC87137AE6CAD3881127056C755AFD7A2DECE8731D9DF6FA` |

Independent post-migration verification (LCD, 08/19): code_id **11589** ✓ ·
