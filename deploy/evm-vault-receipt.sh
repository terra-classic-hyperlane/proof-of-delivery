#!/usr/bin/env bash
# =============================================================================
# tc-proof-of-delivery · TRUSTLESS RECEIPT Vault (EVM) — deploy + LOCAL config
#
#   usage:  PRIVATE_KEY=0x... bash deploy/evm-vault-receipt.sh bsc
#
# Deploys the vault with send_receipt/handle (the v2-attestation becomes deprecated),
# points IGP.beneficiary at it and configures the <chain>↔TC corridor:
#   - operatorAddress[0][local]  = local executor  (feeds reverse-lookup)
#   - operatorAddress[0][132556] = operator address on TC (registration)
#   - remoteRouter[132556]       = TC vault (canonical 32B) — trusts/dispatches
#   - remoteReward[132556]       = real origin fee BSC→TC (BNB)
# Idempotent via deploy/evm-receipt-<chain>.state. NONE of this on the VPS.
# =============================================================================
set -euo pipefail
CHAIN="${1:?usage: evm-vault-receipt.sh bsc|ethereum}"
...
case "$CHAIN" in
  bsc)
    ...
    WARP_ISM="0xF6b0cDD33A7d2895a3F18b85569Ed9A8278cD151"   # warp ISM for TC→BSC (mutable, 3-of-4 — 2026-08-20)
  ...
  ethereum)
    ...
    WARP_ISM="0x3ba17675f0D319C89D70722f6eb07790DF0B254B"   # warp ISM for TC→ETH (mutable, 3-of-4 — 2026-08-20)
...
if ! done_step VAULT; then
  say "1/7 deploy of the vault (receipt) — constructor with localDomain=$LOCAL_DOMAIN"
  ...
  done_step VAULT || { echo "❌ deploy did not confirm at $predicted"; exit 1; }
fi
...
done_step OP_TC || { say "4/7 operatorAddress[0][132556] = $OPERADOR_TC (registration)"; \
  SEND "$V" "setOperatorAddress(uint32,uint32,string)" 0 "$TC_DOMAIN" "$OPERADOR_TC" >/dev/null; mark OP_TC ok; }
done_step ROUTER || { say "5/7 remoteRouter[132556] = TC vault (canonical)"; \
  SEND "$V" "setRemoteRouter(uint32,bytes32)" "$TC_DOMAIN" "$TC_VAULT_HEX32" >/dev/null; mark ROUTER ok; }
done_step ISM || { say "5b/7 setIsm = $WARP_ISM (the same warp ISM — validates receipts coming from TC)"; \
  SEND "$V" "setIsm(address)" "$WARP_ISM" >/dev/null; mark ISM ok; }

if ! done_step REWARD; then
  say "6/7 remoteReward[132556] = real fee BSC→TC (custom IGP formula)"
