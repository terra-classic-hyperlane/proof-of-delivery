#!/usr/bin/env bash
# =============================================================================
# tc-proof-of-delivery · Vault v2 (ClaimRemote) — LOCAL migration on TC
#
#   usage:  KEY=<key_name_in_terrad> bash deploy/tc-migrate-vault-v2.sh
#
# Does: store the v2 wasm -> migrate AT THE SAME address (pool/beneficiary intact)
# -> configures attestor + bindings of the 3 chains + rewards (33 LUNC/domain)
# -> attests the 3 deliveries of 08/19 (SOL/BSC/ETH) = 99 LUNC to the operator.
# Idempotent via deploy/tc-v2.state. Signs with the LOCAL KEYRING key of
# terrad (the owner/admin terra1run9wz…). NONE of this runs on the VPS (project rule).
# =============================================================================
set -euo pipefail
KEY="${KEY:-hyperlane-deploy}"   # same key as tc-deploy.sh (keyring file)
KEYRING="file"
...
echo "wasm: $(sha256sum "$WASM" | cut -d' ' -f1) (expected e24a5e66ab4a503c6acf369710b717310362d2ae5fa7b9800542c8272b2fc801)"

if ! done_step CODE_ID; then
  say "1/4 store the v2 wasm"
  H=$(sign tx wasm store "$WASM" "${TX[@]}" | python3 -c 'import json,sys;print(json.load(sys.stdin)["txhash"])')
  R=$(wait_tx "$H")
  CODE=$(echo "$R" | python3 -c '
import json,sys
r=json.load(sys.stdin)
assert r.get("code")==0, r.get("raw_log","tx failed")
for e in r["events"]:
...
if ! done_step MIGRATED; then
  say "2/4 migrate (same address — pool and beneficiary preserved)"
...
say "3/4 remote configuration (attestor · TC<->BSC/ETH/SOL bindings · rewards)"
...
say "4/4 attest the 3 deliveries of 08/19 (SOL · BSC · ETH) — 33 LUNC each"
...
say "V2 ACTIVE — 99 LUNC paid for today's 3 remote deliveries 🎉"
