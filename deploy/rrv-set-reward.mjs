// rrv-set-reward — ajusta config.reward_lamports do pod (recompensa por entrega
// TC→Solana no modelo de quórum) via governança (SubmitAdminAction → SetRewardLamports).
// Quórum atual = 1 → a aprovação do signatário (operador) executa na hora.
...
const NONCE = BigInt(process.env.NONCE ?? Math.floor(Date.parse("2026-08-19") / 1000)); // fixo p/ evitar Date.now
...
// AdminEnvelope { nonce u64, action: SetRewardLamports(u64)=variante 0 }
...
// [módulo rrv=0][SubmitAdminAction=3] + borsh(envelope)
...
console.log("signatário:", kp.publicKey.toBase58(), "· reward =", REWARD.toString(), "lamports · nonce", NONCE.toString());
